...
export default function HowItWorksSection() {
  ...
  return (
    <section id="how-it-works" className="py-12 md:py-20 px-4 md:px-6 bg-gray-50">
          ...
          {steps.map((step, index) => (
            <div key={index} className="text-center group">
              <div className="relative mb-4 md:mb-6">
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-500 flex items-center justify-center mx-auto group-hover:scale-110 transition-transform duration-300">
                  <step.icon className="w-7 h-7 md:w-8 md:h-8 text-white" />
                </div>
                <div className="absolute -top-2 -right-2 w-5 h-5 md:w-6 md:h-6 rounded-full bg-gray-900 text-white text-xs flex items-center justify-center font-medium">
                  {index + 1}
                </div>
              </div>
              <h3 className="text-lg md:text-xl font-medium text-gray-900 mb-2">{step.title}</h3>
